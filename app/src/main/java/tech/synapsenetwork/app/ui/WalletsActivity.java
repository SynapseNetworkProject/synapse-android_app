...
import tech.synapsenetwork.app.ui.widget.adapter.WalletsAdapter;
import tech.synapsenetwork.app.util.KeyboardUtils;
import tech.synapsenetwork.app.viewmodel.WalletsViewModel;
import tech.synapsenetwork.app.viewmodel.WalletsViewModelFactory;
import tech.synapsenetwork.app.widget.AddWalletView;
import tech.synapsenetwork.app.widget.BackupView;
import tech.synapsenetwork.app.widget.BackupWarningView;
...
public class WalletsActivity extends BaseActivity implements
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		setContentView(tech.synapsenetwork.app.R.layout.activity_wallets);
		// Init toolbar
		toolbar();
		...
		backupWarning = findViewById(tech.synapsenetwork.app.R.id.backup_warning);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == Constants.IMPORT_REQUEST_CODE) {
			showToolbar();
		    if (resultCode == RESULT_OK) {
                viewModel.fetchWallets();
                Snackbar.make(systemView, getString(tech.synapsenetwork.app.R.string.toast_message_wallet_imported), Snackbar.LENGTH_SHORT)
                        .show();
            ...
            if (resultCode == RESULT_OK) {
                Snackbar.make(systemView, getString(tech.synapsenetwork.app.R.string.toast_message_wallet_exported), Snackbar.LENGTH_SHORT)
                        .show();
                backupWarning.hide();
                showToolbar();
                hideDialog();
                ...
                dialog = buildDialog()
                        .setMessage(tech.synapsenetwork.app.R.string.do_manage_make_backup)
                        .setPositiveButton(tech.synapsenetwork.app.R.string.yes_continue, (dialog, which) -> {
                            hideDialog();
                            backupWarning.hide();
                            showToolbar();
                            if (adapter.getItemCount() <= 1) {
                        ...
                        .setNegativeButton(tech.synapsenetwork.app.R.string.no_repeat,
                                (dialog, which) -> {
	...
	private void onFetchWallet(Wallet[] wallets) {
		if (wallets == null || wallets.length == 0) {
			...
			AddWalletView addWalletView = new AddWalletView(this, tech.synapsenetwork.app.R.layout.layout_empty_add_account);
			addWalletView.setOnNewWalletClickListener(this);
			addWalletView.setOnImportWalletClickListener(this);
    ...
    private void showNoBackupWarning(Wallet wallet) {
        dialog = buildDialog()
                .setTitle(getString(tech.synapsenetwork.app.R.string.title_dialog_watch_out))
                .setMessage(getString(tech.synapsenetwork.app.R.string.dialog_message_unrecoverable_message))
                .setIcon(tech.synapsenetwork.app.R.drawable.ic_warning_black_24dp)
                .setPositiveButton(tech.synapsenetwork.app.R.string.i_understand, (dialog, whichButton) -> {
                    backupWarning.hide();
                    showToolbar();
                })
    ...
    private void showBackupDialog(Wallet wallet, boolean isNew) {
        ...
        dialog = buildDialog()
                .setView(view)
                .setPositiveButton(tech.synapsenetwork.app.R.string.ok,
                        (dialogInterface, i) -> {
                            viewModel.exportWallet(wallet, view.getPassword());
                            KeyboardUtils.hideKeyboard(view.findViewById(tech.synapsenetwork.app.R.id.password));
                        })
                .setNegativeButton(tech.synapsenetwork.app.R.string.cancel, (dialogInterface, i) -> {
                    if (isNew) {
                        onCreatedWallet(wallet);
                    }
                    KeyboardUtils.hideKeyboard(view.findViewById(tech.synapsenetwork.app.R.id.password));
                })
                .setOnDismissListener(dialog -> KeyboardUtils.hideKeyboard(view.findViewById(tech.synapsenetwork.app.R.id.password)))
                .create();
        dialog.show();
        handler.postDelayed(() -> {
            KeyboardUtils.showKeyboard(view.findViewById(tech.synapsenetwork.app.R.id.password));
        }, 500);
	...
	private void onDeleteWallet(Wallet wallet) {
		dialog = buildDialog()
				.setTitle(getString(tech.synapsenetwork.app.R.string.title_delete_account))
				.setMessage(getString(tech.synapsenetwork.app.R.string.confirm_delete_account))
				.setIcon(tech.synapsenetwork.app.R.drawable.ic_warning_black_24dp)
