...
public class GasSettingsActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        BigInteger gasPrice = new BigInteger(getIntent().getStringExtra(Constants.EXTRA_GAS_PRICE));
        BigInteger gasLimit = new BigInteger(getIntent().getStringExtra(Constants.EXTRA_GAS_LIMIT));
        BigInteger gasLimitMin = BigInteger.valueOf(Constants.GAS_LIMIT_MIN);
        BigInteger gasLimitMax = BigInteger.valueOf(Constants.GAS_LIMIT_MAX);
        BigInteger gasPriceMin = BigInteger.valueOf(Constants.GAS_PRICE_MIN);
        BigInteger networkFeeMax = BigInteger.valueOf(Constants.NETWORK_FEE_MAX);
    ...
    private void onDefaultNetwork(NetworkInfo network) {
        gasPriceInfoText.setText(getString(R.string.info_gas_price).replace(Constants.ETHEREUM_NETWORK_NAME, network.name));
        gasLimitInfoText.setText(getString(R.string.info_gas_limit).replace(Constants.ETHEREUM_NETWORK_NAME, network.symbol));
    }

    private void onGasPrice(BigInteger price) {
        String priceStr = BalanceUtils.weiToGwei(price) + " " + Constants.GWEI_UNIT;
    ...
    private void updateNetworkFee() {
        String fee = BalanceUtils.weiToEth(viewModel.networkFee()).toPlainString() + " " + Constants.ETH_SYMBOL;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_save: {
                ...
                intent.putExtra(Constants.EXTRA_GAS_PRICE, viewModel.gasPrice().getValue().toString());
                intent.putExtra(Constants.EXTRA_GAS_LIMIT, viewModel.gasLimit().getValue().toString());
